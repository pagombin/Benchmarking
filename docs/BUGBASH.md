...
| B-027 | P3 | G (user report) | .card / Continuous fleet board | Wide tables overflowed their card's border — `.card` had no overflow handling and `table {width:100%}` can't shrink below content, so the fleet board's row actions rendered on the page background outside the card | Cards clip and scroll horizontally (`overflow-x: auto`; nothing in a card overlays past its edge); the fleet table additionally tightens padding (`table.dense`), narrows the spark to 90px and truncates the last-alert badge (full text in tooltip) so Stop/Resume stays on-screen at common widths | Verified in headless Chromium at 1440px: action button's right edge inside the card bounds (was 45px past it) |
